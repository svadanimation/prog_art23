...
def buildSubmitDict(make_movie=False, project=False, high_memory=0):
    '''
    ...
    if not len(sceneFile):
        mc.warning('Save file before submitting render!')
        mel.eval("SaveSceneAs")
    if sceneFile.startswith(constants.OMIT_DRIVES):
        mc.error('File must be on a network share')

    # reconstruct shot name
    shotNameTokens = os.path.basename(sceneFile).split('.')
    shotNameTokens.pop(-1)
    shotName = '_'.join(shotNameTokens)
    
    #paths and names
    jobName = 'vray: ' + shotName
    ext = 'png'
    if mc.getAttr('vraySettings.imageFormatStr') is not None:
        ext = mc.getAttr('vraySettings.imageFormatStr').split(' ', -1)[0]
    dirPath = os.path.dirname(os.path.abspath(sceneFile))
    renderPath = os.path.join(dirPath, 'render')
    framePath = os.path.join(renderPath, (shotName + '.#.' + ext))
    vrscene = os.path.join(renderPath, (shotName + '.vrscene'))
    ...
    driveTable = render_utils.uncDriveTable(remove = constants.NETWORK_SUFFIX)
    renderPath = render_utils.uncMapper(renderPath, driveTable=driveTable)
    framePath = render_utils.uncMapper(framePath, driveTable=driveTable)
    vrscene = render_utils.uncMapper(vrscene, driveTable=driveTable)
    ...
    #build dictionary
    # the easiest way to find this is to look at qube job properties and job internals to get the key names
    jobs = {}
    vray_job = {}
    ...
    # these two keys work in combination. specifying the cluster doesn't ensure a job won't run a worker, just ajusts the priority
    ...
    vray_job['renderpath'] = renderPath
    vray_job['label'] = 'render'
    vray_job['name'] = jobName
    ...
    # this esoteric flag is to make the job do worker path translation. Every path to be converted according to the qbwork.conf needs to be wrapped in QB_constants.CONVERT_PATH()
    vray_job['flags'] = '131072'
    
    # on osx this is controlled via the vray.sh file which spins these up just in time and then passes any additional flags
    vray_job['env'] = {
        'PATH': '%PATH%;{}'.format(constants.VRAY_PATH),
    ...
    cmd = (
        ...
        f' -imgWidth={width}'
        f' -imgHeight={height}'
        f' -imgFile="QB_constants.CONVERT_PATH({str(framePath)})"'
        ' -showProgress=0'
        ' -display=0'
    )
      
    # the basic format for submission
    #package['cmd_template'] =  '%(vray_path)s -frames=QB_FRAME_START-QB_FRAME_END,QB_FRAME_STEP -autoClose=1 %(argv)s' 
    package['cmdline'] = cmd    
    ...
    # Allow job to run on all stations without a prioity hit. It will try to run on the /osx cluster first
    if project:
        vray_job['cluster']='/osx/+'
        vray_job['requirements'] = 'host.os=osx || host.os =winnt'
        vray_job['project'] = project
        
    # Consider exposing this memory value in confirmation script, anything more than 10k won't run on old farm
    if high_memory:        
    ...
    if make_movie:             
        ...
        basePath = framePath.split('#',1)[0]
        
        # Movie agenda has a single task
        agenda_movie = qb.genframes('1')
        # Set the outputPaths in the resultpackage for each agenda item, this is for compatability with shotgun_submitVersion.py
        for i in agenda_movie:
            i['resultpackage']={'outputPaths': str(basePath + 'thumb.jpg')}
        movie_job['agenda'] = agenda_movie
        movie_job['name'] = str('pdplayer: ' + shotName)
        movie_job['package'] = {}
        ...
        pdplayercmd = (
            ...
            ' --mask_type=crop'
            ' --fps=24'
            f' --save_mask_as_image={str(basePath +  "thumb.jpg")}'
            f' --save_mask_as_sequence={str(basePath + "movie.mov")},mp4v,100'
            ' --exit'
        )

        movie_job['package']['cmdline'] = pdplayercmd
        movie_job['package']['thumbnail'] = str(basePath +  'thumb.jpg')
        movie_job['package']['movie'] = str(basePath + 'movie.mov')
...
def vray_standalone(make_movie=False, project=False, show_UI=True):
    ...
    #fill the dictionary   
    jobs = buildSubmitDict(make_movie=make_movie, project=project)

    if show_UI:
        jobs = submit_UI.validateQubeDictUI(jobs)
        vray_standalonePost(jobs)

    else:
        vray_standalonePost(jobs, show_UI=False)
 
def vray_standalonePost(jobs, show_UI=True):
    ...
    vrayMeshes = mc.ls(type='VRayMesh')
    vrayMeshStatus = list()

    if vrayMeshes:
        print ('Temporarily disabling vrayMesh preview')
        for vrayMesh in vrayMeshes:
            vrayMeshStatus.append((vrayMesh, mc.getAttr(vrayMesh + '.geomType') ))
            mc.setAttr(vrayMesh + '.geomType', 0)
            
    # build path
    if not render_utils.build_directory(filepath=renderPath):
        mc.error(f'Failed to build directory: {renderPath}')

    #If this is a project job, it needs to be cross-platform, so we will replace all the paths with the UNC equivalent
    ...
    originalPostTranslatePython = mc.getAttr('vraySettings.postTranslatePython')
    postTranslatePython = (
    ...
    if project:
        ...
        mc.setAttr('vraySettings.postTranslatePython', originalPostTranslatePython+postTranslatePython, type='string')    

    # create a temporary vray settings node that mimics the true settings node except for some options
    if mc.objExists("vraySettings_qubeExport"):
        mc.delete("vraySettings_qubeExport")
        
    mc.createNode("VRaySettingsNode", shared = True, n = "vraySettings_qubeExport")
    mc.copyAttr("vraySettings", "vraySettings_qubeExport", inConnections = True, values = True, ksc=True)
    ...
    # cleanup
    mc.delete("vraySettings_qubeExport")
    mc.setAttr('vraySettings.postTranslatePython', originalPostTranslatePython, type='string')
    ...
    if vrayMeshStatus:
        ...
        for vrayMesh in vrayMeshStatus:
            mc.setAttr(vrayMesh[0] + '.geomType', vrayMesh[1])
    ...
    resultString = ''
    for j in submitted:
        resultString += f'Submitted: {j["id"]} = {j["name"]} \n'
    
    print(resultString)

    if show_UI:
        mc.confirmDialog( title='Qube submit report', message=resultString, button='Ok', defaultButton='Ok' )
